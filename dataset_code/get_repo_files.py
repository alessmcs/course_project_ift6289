...
def get_repo_contents(repo_metadata):
    ###
    # Get all files in the repo using Git Trees API, then filter based on rules:
    # - If 'src' directory exists, only include files under 'src/'
    # - Otherwise, include all files from the root (but still exclude based on EXCLUDED_DIR_NAMES)
    # This way we can capture more files that are relevant for code understanding, while avoiding tests, docs, examples, and other non-code files.
    ###
...
def get_latest_commit_sha(owner_repo, branch, until_date=None):
    ###
    # Get the latest commit SHA for the given repo and branch, optionally filtering by an "until" date.
    ###
