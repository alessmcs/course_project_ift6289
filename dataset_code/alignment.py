...
# Steps: 
# 1. Build a code index with ASTs, (file path, start/end lines, full code text)
# 2. Perform strong alignment for docstrings and javadocs
# 3. Perform medium alignment for inline comments 
# 4. Align commit messages to files (Keep: new_start -> new_start + new_len)
...
FINAL_OUTPUT_FILE = "final_aligned_dataset2.jsonl"
...
# Patch parsing (commit log alignment)
...
def main():
    ...
    # 1. Filter Repos (buggy - skip)
    # scores = load_json(REPO_SCORES_FILE)
    # filtered_repos = [s for s in load_jsonl(ACCEPTED_REPOS_FILE) if s["scores"]["final_score"] >= 0.55]

    filtered_repos = load_jsonl(ACCEPTED_REPOS_FILE) 

    random.seed(42) # for reproducibility
    #filtered_repos = random.sample(filtered_repos[:100], 15) # for testing 
    ...
    for repo_id in filtered_repo_ids:
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(resp.content)) as z:
                ...
                # only focus on files that have entities to align for efficiency (& core files)
                needed_files = set()
                ...
                # Strong alignment: docstrings/javadocs
                # Medium alignment: inline comments 
                for ent in repo_file_entities.get(repo_id, []):
                    path = ent["file_path"]
