...
import dataset_code.file_metrics as file_metrics
...
repo_scores_file = "repo_scores.jsonl"
...
def main():
    ...
    # (resume logic) Check which repos are already scored
    processed_repos = set()
    if os.path.exists(repo_scores_file):
        with open(repo_scores_file, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    try:
                        processed_repos.add(json.loads(line)["repo_id"])
                    except: continue

    print(f"[INFO] Resuming... Found {len(processed_repos)} already processed repositories.")
    ...
    for repo_id, repo_row in repo_map.items():
        ...
        # Save score for this repo immediately
        with open(repo_scores_file, "a", encoding="utf-8") as f:
